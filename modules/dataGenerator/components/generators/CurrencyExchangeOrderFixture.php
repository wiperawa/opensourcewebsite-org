...
class CurrencyExchangeOrderFixture extends ARGenerator
{
    ...
    protected function factoryModel(): ?ActiveRecord
    {
        ...
        [$orderLat, $orderLon] = $this->generateRandomPoint($londonCenter, 10);

        $cross_rate_on = (int)static::getFaker()->boolean();

        return new CurrencyExchangeOrder([
            'selling_currency_id' => $currencies[0],
            'buying_currency_id' => $currencies[1],
            'user_id' => $user->id,
            'selling_rate' => $cross_rate_on?null:static::getFaker()->valid(static function ($v) { return (bool)$v; })->randomFloat(1, 0.01, 10),
            ...
            'delivery_radius' => static::getFaker()->randomNumber(3),
            'location_lat' => $orderLat,
            'location_lon' => $orderLon,
            'selling_cash_on' => (int)static::getFaker()->boolean(),
            'buying_cash_on' => (int)static::getFaker()->boolean(),
            'cross_rate_on' => $cross_rate_on,
            ]);
    }
